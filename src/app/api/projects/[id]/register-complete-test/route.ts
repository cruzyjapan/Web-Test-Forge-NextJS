...
export async function POST(
  ...
  try {
    ...
    if (selectedTabs?.['page-tests'] !== false && analysisResult?.pages && Array.isArray(analysisResult.pages)) {
      ...
      const nonApiPages = analysisResult.pages.filter((page: any) => {
      ...
      for (let i = 0; i < nonApiPages.length; i++) {
        ...
        try {
          ...
          if (page.links && page.links.length > 0) {
            // 重複を除去
            const uniqueLinks = [...new Set(page.links)] as string[];
    ...
    if (selectedTabs?.['form-tests'] !== false && analysisResult?.forms && Array.isArray(analysisResult.forms)) {
      ...
      for (let formIndex = 0; formIndex < analysisResult.forms.length; formIndex++) {
        ...
        try {
          ...
          // 個別フィールドのバリデーションテスト
          if (form.elements && form.elements.length > 0) {
            for (const element of form.elements.filter((e: any) => e.required)) {
    ...
    if (selectedTabs?.['navigation-tests'] !== false && analysisResult?.navigation && Array.isArray(analysisResult.navigation)) {
      ...
      // 重複を除去してソート
      const uniqueLinks = [...new Set(analysisResult.navigation)].sort() as string[];
    ...
    if (false && analysisResult?.routes && Array.isArray(analysisResult.routes)) {
      const apiRoutes = analysisResult.routes.filter((r: any) => r.path.startsWith('/api'));
    ...
    if (analysisResult?.routes && Array.isArray(analysisResult.routes)) {
      const dynamicRoutes = analysisResult.routes.filter((r: any) => r.params && r.params.length > 0);
    ...
    console.log(`Test breakdown (based on selected tabs):
      ...
      - Dynamic route tests: ${analysisResult?.routes?.filter((r: any) => r.params?.length > 0).length || 0}
    `);
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        dynamicRoutes: analysisResult?.routes?.filter((r: any) => r.params?.length > 0).length || 0,
