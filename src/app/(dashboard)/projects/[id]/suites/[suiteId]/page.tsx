...
export default function SuiteDetailPage() {
  ...
  const getStatusIcon = (caseId: string, testCase: TestCase) => {
    ...
    if (config.skipped) {
      return <span title={config.skipReason || (language === 'ja' ? 'スキップ' : 'Skipped')}><SkipForward className="h-4 w-4 text-yellow-500" /></span>;
    }
    
    const status = testCaseStatuses[caseId];
    if (!status) {
      return <span title={language === 'ja' ? "未実行" : "Not executed"}><Clock className="h-4 w-4 text-gray-400" /></span>;
    }
    
    switch (status.status) {
      case 'success':
        return <span title={language === 'ja' ? "成功" : "Success"}><CheckCircle className="h-4 w-4 text-green-500" /></span>;
      case 'failed':
        return <span title={language === 'ja' ? "失敗" : "Failed"}><XCircle className="h-4 w-4 text-red-500" /></span>;
      case 'running':
        return <span title={language === 'ja' ? "実行中" : "Running"}><Loader2 className="h-4 w-4 text-blue-500 animate-spin" /></span>;
      case 'not_executed':
      default:
        return <span title={language === 'ja' ? "未実行" : "Not executed"}><Clock className="h-4 w-4 text-gray-400" /></span>;
    }
